...
public class VaadinBundleTracker extends BundleTracker<Bundle> {
	...
	private static Logger logger = LoggerFactory.getLogger(VaadinBundleTracker.class.getName());
	...
	public VaadinBundleTracker(BundleContext context) {
		...
		logger.debug(VaadinBundleTracker.class.getName()+" activated via Constructor.");
	...
	private void registerPushResources(Bundle pushBundle) {
		...
		properties.put(HttpWhiteboardConstants.HTTP_WHITEBOARD_SERVLET_PATTERN,
				"/VAADIN/static/push/*");
		servletPushRegistration.compareAndSet(null, pushBundle.getBundleContext()
				.registerService(Servlet.class,
						new ResourceServlet(pushBundle, "/META-INF/resources/VAADIN/static/push"), properties));
	}

	private void unregisterPushResource(Bundle pushBundle) {

		ServiceRegistration<Servlet> registration = servletPushRegistration.get();
		if (registration != null
				&& registration.getReference().getBundle().getBundleId() == pushBundle.getBundleId()) {
	...
	private void registerClientResources(Bundle clientBundle) {
		...
		Hashtable<String, Object> properties = new Hashtable<>();
		properties.put("osgi.http.whiteboard.servlet.pattern", "/VAADIN/static/client/*");
		servletClientRegistration.compareAndSet(null,
				clientBundle.getBundleContext()
						.registerService(Servlet.class,
								new ResourceServlet(clientBundle, "/META-INF/resources/VAADIN/static/client"),
								properties));
	}

	private void unregisterClientResource(Bundle clientBundle) {

		ServiceRegistration<Servlet> registration = servletClientRegistration.get();
		if (registration != null
				&& registration.getReference().getBundle().getBundleId() == clientBundle.getBundleId()) {
			registration.unregister();
	...
	private void scanContextInitializers() {
		...
		Collection<Class<? extends ServletContainerInitializer>> initializers = classes.stream()
	...
	private void scanActiveBundles() {
		...
		Stream.of(flowServerBundle.getBundleContext().getBundles())
				...
				.filter(this::isVaadinExtender)
				.forEach(activeBundle -> scanClasses(activeBundle, map, this::handleBundleClassError));
		OSGiAccess.getInstance().addScannedClasses(map);
	}
	...
	private boolean isVaadinExtender(Bundle bundle) {
		...
		return !flowServerBundle.equals(bundle)
				&& Boolean.TRUE.toString().equals(bundle.getHeaders().get("Vaadin-OSGi-Extender"));
	}
	...
	private void scanClasses(Bundle bundle, Map<Long, Collection<Class<?>>> map,
		...
		for (String clazz : classes) {
			...
			if (bundle.equals(flowServerBundle) && !className.startsWith("com.vaadin")) {
				continue;
			}
